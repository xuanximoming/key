using System;

namespace DrectSoft.Core.Permission
{
    public class UsersEntity
    {
        #region declaration
        private string m_Id = String.Empty;
        private string m_Name = String.Empty;
        private string m_Py = String.Empty;
        private string m_Wb = String.Empty;
        private string m_Sexy = String.Empty;
        private string m_Birth = String.Empty;
        private string m_Marital = String.Empty;
        private string m_Idno = String.Empty;
        private string m_Deptid = String.Empty;
        private string m_Wardid = String.Empty;
        private string m_Category = String.Empty;
        private string m_Jobtitle = String.Empty;
        private string m_Recipeid = String.Empty;
        private int m_Recipemark = 0;
        private int m_PowerMark = 0;
        private int m_Narcosismark = 0;
        private string m_Groupid = String.Empty;
        private string m_Grade = String.Empty;
        private string m_Passwd = String.Empty;
        private string m_Jobid = String.Empty;
        private string m_DeptOrWard = String.Empty;
        private string m_Regdate = String.Empty;
        private string m_Operator = String.Empty;
        private int m_Onlinestate = 0;
        private int m_Status = 0;
        private int m_Valid = 0;
        private string m_Memo = String.Empty;
        #endregion declaration

        public UsersEntity()
        {
        }

        #region Properties
        public string Id
        {
            get
            {
                return m_Id;
            }
            set
            {
                m_Id = value;
            }
        }
        public string Name
        {
            get
            {
                return m_Name;
            }
            set
            {
                m_Name = value;
            }
        }
        public string Py
        {
            get
            {
                return m_Py;
            }
            set
            {
                m_Py = value;
            }
        }
        public string Wb
        {
            get
            {
                return m_Wb;
            }
            set
            {
                m_Wb = value;
            }
        }
        public string Sexy
        {
            get
            {
                return m_Sexy;
            }
            set
            {
                m_Sexy = value;
            }
        }

        public string Birth
        {
            get
            {
                return m_Birth;
            }
            set
            {
                m_Birth = value;
            }
        }
        public string Marital
        {
            get
            {
                return m_Marital;
            }
            set
            {
                m_Marital = value;
            }
        }
        public string Idno
        {
            get
            {
                return m_Idno;
            }
            set
            {
                m_Idno = value;
            }
        }
        public string Deptid
        {
            get
            {
                return m_Deptid;
            }
            set
            {
                m_Deptid = value;
            }
        }
        public string Wardid
        {
            get
            {
                return m_Wardid;
            }
            set
            {
                m_Wardid = value;
            }
        }
        public string Category
        {
            get
            {
                return m_Category;
            }
            set
            {
                m_Category = value;
            }
        }
        public string Jobtitle
        {
            get
            {
                return m_Jobtitle;
            }
            set
            {
                m_Jobtitle = value;
            }
        }
        public string Recipeid
        {
            get
            {
                return m_Recipeid;
            }
            set
            {
                m_Recipeid = value;
            }
        }
        public int Recipemark
        {
            get
            {
                return m_Recipemark;
            }
            set
            {
                m_Recipemark = value;
            }
        }

        public int Powermark
        {
            get
            {
                return m_PowerMark;
            }
            set
            {
                m_PowerMark = value;
            }
        }
        public int Narcosismark
        {
            get
            {
                return m_Narcosismark;
            }
            set
            {
                m_Narcosismark = value;
            }
        }
        public string Groupid
        {
            get
            {
                return m_Groupid;
            }
            set
            {
                m_Groupid = value;
            }
        }
        public string Grade
        {
            get
            {
                return m_Grade;
            }
            set
            {
                m_Grade = value;
            }
        }
        public string Passwd
        {
            get
            {
                return m_Passwd;
            }
            set
            {
                m_Passwd = value;
            }
        }
        public string Jobid
        {
            get
            {
                return m_Jobid;
            }
            set
            {
                m_Jobid = value;
            }
        }

        public string DeptOrWard
        {
            get
            {
                return m_DeptOrWard;
            }
            set
            {
                m_DeptOrWard = value;
            }
        }
        public string Regdate
        {
            get
            {
                return m_Regdate;
            }
            set
            {
                m_Regdate = value;
            }
        }
        public string Operator
        {
            get
            {
                return m_Operator;
            }
            set
            {
                m_Operator = value;
            }
        }
        public int Onlinestate
        {
            get
            {
                return m_Onlinestate;
            }
            set
            {
                m_Onlinestate = value;
            }
        }
        public int Status
        {
            get
            {
                return m_Status;
            }
            set
            {
                m_Status = value;
            }
        }
        public int Valid
        {
            get
            {
                return m_Valid;
            }
            set
            {
                m_Valid = value;
            }
        }
        public string Memo
        {
            get
            {
                return m_Memo;
            }
            set
            {
                m_Memo = value;
            }
        }
        #endregion Properties
    }

}
