...
using DrectSoft.Emr.QCTimeLimit.QCControlUse;
...
namespace DrectSoft.Core.TimeLimitQC
{
    ...
    /// 时限质量查询窗口
    /// </summary>
    public partial class FormTimeQCInfo : Form, IStartPlugIn
    {
        #region fields
        ...
        public void FormTimeQCInfo_Load(object sender, EventArgs e)
        {
            ...
            ucTimeQcInfo1.App = app;
            ucTimeQcInfo1.CheckDoctorLimit(app.User.Id);
            //初始化科室、医生下拉框数据
            InitDept();
            InitDoc(lookUpEditorDept.CodeValue);
        ...
        public DataTable GetGridViewBaseData()
        {
            try
            {
                string sqlStr = @"select a.ID,a.ConditionTime,a.FoulState,REALCONDITIONTIME,a.ResultTime,a.Reminder,a.FoulMessage,b.Name as PatName,b.sexid,case b.sexid when '1' then '男' when '2' then '女' else '未知' end as Sex,b.Age,b.NoOfInpat,b.NoOfRecord,b.PatID,b.Outbed,b.inwarddate as InTime,c.id UserID,c.Name UserName,c.DeptID,p.name as DeptName,d.TimeLimit 
                              from QCRecord a left join InPatient b on a.NoOfInpat=b.NoOfInpat left join QCRule d on a.RuleCode=d.RuleCode  left join Users c on b.resident = c.ID left join department p on c.deptid=p.id 
                              where a.Valid!=0 and a.FoulState in (1,3) and d.RuleCode is not null ";
                if (!string.IsNullOrEmpty(lookUpEditorDept.CodeValue))
                {
                    sqlStr += " and c.deptid = '" + lookUpEditorDept.CodeValue + "'";
                }
        ...
        public DataTable CreateDataTable()
        {
            try
            {
                ...
                newTable.Columns.Add(new DataColumn("Outbed", typeof(string)));
        ...
        private void xtraTabControl1_SelectedPageChanged(object sender, DevExpress.XtraTab.TabPageChangedEventArgs e)
        {
            try
            {
                ...
                {
                    this.ActiveControl = ucTimeQcInfo1;
                }
