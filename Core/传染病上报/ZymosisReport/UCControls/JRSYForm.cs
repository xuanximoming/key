...
namespace DrectSoft.Core.ZymosisReport.UCControls
{
    ...
    /// 尖锐湿疣界面 
    /// </summary>
    public partial class JRSYForm : DevExpress.XtraEditors.XtraUserControl, IFuCade
    {
        string m_reportID;
        DataRow m_dataRow;
        public JRSYForm(string reportID)
        {
            m_reportID = reportID;
            InitializeComponent();
        ...
        public DataRow GetDateRow(string reportID)
        {
            ...
            {
                //数据库中查询出DataRow
                string sql = @"select * from ZYMOSIS_JRSY t where t.vaild='1' and  t.report_id=" + reportID;
        ...
        private void SaveData()
        {
            try
            {
                ...
                parameters[0] = new SqlParameter("@jrsy_id", SqlDbType.VarChar);
                ...
                parameters[9] = new SqlParameter("@contacthistory", SqlDbType.VarChar);
                parameters[10] = new SqlParameter("@venerismhistory", SqlDbType.VarChar);
                parameters[11] = new SqlParameter("@infactway", SqlDbType.VarChar);
                parameters[12] = new SqlParameter("@samplesource", SqlDbType.VarChar);
                parameters[13] = new SqlParameter("@detectionconclusion", SqlDbType.VarChar);
                parameters[14] = new SqlParameter("@affirmdate", SqlDbType.VarChar);
                parameters[15] = new SqlParameter("@affirmlocal", SqlDbType.VarChar);
                parameters[16] = new SqlParameter("@vaild", SqlDbType.VarChar);
                parameters[17] = new SqlParameter("@creator", SqlDbType.VarChar);
                parameters[18] = new SqlParameter("@createdate", SqlDbType.VarChar);
                parameters[19] = new SqlParameter("@mender", SqlDbType.VarChar);
                parameters[20] = new SqlParameter("@alterdate", SqlDbType.VarChar);

                if (m_dataRow != null && !string.IsNullOrEmpty(m_dataRow["JRSY_ID"].ToString()))
                {
                    parameters[0].Value = m_dataRow["JRSY_ID"].ToString();
                }
                else
                {
                ...
                parameters[17].Value = DrectSoft.Common.DS_Common.currentUser.DoctorId;
                parameters[18].Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                parameters[19].Value = DrectSoft.Common.DS_Common.currentUser.DoctorId;
                parameters[20].Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                ...
                DrectSoft.DSSqlHelper.DS_SqlHelper.ExecuteNonQuery("EMR_ZYMOSIS_REPORT.usp_AddOrModJRSYReport", parameters, CommandType.StoredProcedure);
            }
            catch (Exception ex)
            {
