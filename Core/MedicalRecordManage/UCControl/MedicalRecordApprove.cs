...
namespace MedicalRecordManage.UCControl
{
    ...
    /// 病历借阅审核界面
    /// </summary>
    public partial class MedicalRecordApprove : DevExpress.XtraEditors.XtraUserControl//, IEMREditor
    {
        //IEmrHost m_app;
        ...
        private void btnApprove_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("请选择需要审核的借阅记录 ", "信息提示");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (dR == DialogResult.OK)
                {
                    for (int i = 0; i < selection.SelectedCount; i++)
                    {
                        ...
                        if (flag == 2)
                        {
                            CApplyObject.Approve(id, m_sUser);
                        }
                        else if (flag == 3)
                        {
                            CApplyObject.ApproveRef(id, m_sUser, memoReason.Text.Trim());
                        }
                    }
        ...
        private void LoadData()
        {
            try
            {
                ...
                string sql = MedicalRecordManage.Object.DaoCommand._SELECT_MEDICAL_RECORD_BORROW_SQL + " and i.status = 1";
                if (valueStr.ToLower().Contains("iem_mainpage_diagnosis_sx"))
                {
                    sql = MedicalRecordManage.Object.DaoCommand._SELECT_MEDICAL_RECORD_BORROW_SQL_ZY + " and i.status = 1";
                }
