...
namespace MedicalRecordManage.Object
{
    class CApplyObject
    {
        ...
        /// <returns></returns>
        public static int IsExistApply(CApplyObject applyObject)
        {
            try
            {
                //string sql = " select * from EMR_RecordBorrow where noofinpat = '" + 
                //    applyObject.m_sNoOfInpat + "'" + 
                //    " and status < 2 " +
                //    " and applydocid = '" + applyObject.m_sApplyDocId + "'";
                string sql = @"select * from EMR_RecordBorrow where noofinpat=@nOofinpat 
        ...
        public static int IsCanDelay(CApplyObject applyObject, int times)
        {
            try
            {
                //获取被延期记录是否也是延期记录，是获取记录的申请ID
                string delayid = CApplyObject.GetApplyDelayId(applyObject.m_sApply);
                ...
                {
                    string sql = " select id from EMR_RecordBorrow where yanqiflag = '" +
                        delayid + "'" +
                        " and status in(2,5) " +
                        " and applydocid = '" + applyObject.m_sApplyDocId + "'";
        ...
        public static int GetDelayedApplyTimesResidue(string applyId)
        {
            try
            {
                string sql = " SELECT APPLYTIMES,APPROVEDATE FROM EMR_RecordBorrow WHERE ID = '" +
                    applyId + "'";
                DataSet data = DrectSoft.DSSqlHelper.DS_SqlHelper.ExecuteDataSet(sql);
        ...
        public static int Delay(CApplyObject applyObject)
        {
            try
            {
                //将原有借阅申请剩余的期限加上
                int i = CApplyObject.GetDelayedApplyTimesResidue(applyObject.m_sApply);
                applyObject.m_iApplyTimes = applyObject.m_iApplyTimes + i;
                //原有记录存在延期申请，
                string delayid = CApplyObject.GetApplyDelayId(applyObject.m_sApply);
                if (delayid == null || delayid.Equals(""))
                    delayid = applyObject.m_sApply;
                string sql = @"insert into emr_recordborrow(noofinpat,applydocid,applycontent,applytimes,
        ...
        //获取延期借阅申请延期所指的原有借阅申请的ID
        public static string GetApplyDelayId(string id)
        {
            try
            {
                string sql = " select yanqiflag from EMR_RecordBorrow where id = '" + id + "'";
        ...
        public static void UpdateStatus(string id, int status)
        {
            try
            {
                string sql = " update EMR_RecordBorrow set status = " + status.ToString() +
                    " where id = " + "'" + id + "'";
        ...
        /// </summary>
        /// <param name="applyObject"></param>
        public static void Edit(CApplyObject applyObject)
        {
            try
            {
                //string sql = " update emr_recordborrow set applytimes =" + applyObject.m_iApplyTimes +
                //    ", applycontent = " + "'" + applyObject.m_sApplyContent + "'" +
                //     ", status = " + applyObject.m_iStatus.ToString() +
                //     " where id = " + "'" + applyObject.m_sApply + "'";
                string sql = @"update emr_recordborrow set applytimes=@applyTimes,applycontent=@applyContent,
        ...
        public static void Approve(string id, string user)
        {
            try
            {
                ...
                //string sql = " update EMR_RecordBorrow set status = 2, " +
                //    " approvedocid = '" + user + "', " +
                //    " approvedate = sysdate " +
                //    " where id = " + "'" + id + "'";
                #endregion
                //string sql = string.Format(@"update EMR_RecordBorrow set status = 2,
                //approvedocid='{0}',approvedate=sysdate where id='{1}'", user, id);
                string sql = @"update EMR_RecordBorrow set status='2',approvedocid=@approvedocid,
        ...
        public static void ApproveDealWithDelayRelatedApply(string apply)
        {
            try
            {
                ...
                if (delay != null)
                {
                    string sql = " update EMR_RecordBorrow set status = 5 " +
        ...
        public static void ApproveRef(string id, string user, string message)
        {
            ...
            {
                #region 注销 by xlb 2013-03-29
                //string sql = " update EMR_RecordBorrow set status = 3, " + 
                //    " approvedocid = '" + user + "', " + 
                //    " approvecontent ='" + message + "', " +
                //    " approvedate = sysdate " + 
                //    " where id = " + "'" + id + "'";
                #endregion
                string sql = @"update EMR_RecordBorrow set status='3',approvedocid=@approvedocid,
        ...
        public static void Delete(string id)
        {
            try
            {
                string sql = " delete from EMR_RecordBorrow where id =" + "'" + id + "'";
                DrectSoft.DSSqlHelper.DS_SqlHelper.ExecuteNonQuery(sql);
            }
            catch (Exception)
            {
