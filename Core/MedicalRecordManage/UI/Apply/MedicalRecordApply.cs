...
namespace MedicalRecordManage.UI
{
    ...
    /// 借阅申请界面
    /// </summary>
    public partial class MedicalRecordApply : DevBaseForm//, IEMREditor
    {
        IEmrHost m_app;
        ...
        private void MedicalRecordApply_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rdgPurpose.SelectedIndex != 3)//选择框没选其他时借阅目的文本框不可编辑
                {
                    memoReason.Properties.ReadOnly = true;
                }
                memoReason.Text = rdgPurpose.Properties.Items[rdgPurpose.SelectedIndex].ToString();
            }
            catch (Exception ex)
            {
                MyMessageBox.Show(1, ex);
        ...
        private void rdgPurpose_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rdgPurpose.SelectedIndex == 3/*3表示其他借阅目的*/)
                {
                    memoReason.Properties.ReadOnly = false;//借阅原因文本框可编辑
                    memoReason.Text = "";
                ...
                {
                    memoReason.Text = rdgPurpose.Properties.Items[rdgPurpose.SelectedIndex].ToString();
                    memoReason.Properties.ReadOnly = true;//只读
