...
namespace DrectSoft.Emr.NurseCenter
{
    ...
    public partial class SetPatientsBaby : DevBaseForm
    {
        ...
        public SetPatientsBaby(string NoOfInpat, IEmrHost myapp, string mpatname, NurseForm mnurseform)
        {
            NOOFINPAT = NoOfInpat;
            m_app = myapp;
            PATNAME = mpatname;
            m_NurseForm = mnurseform;
            InitializeComponent();
        }
        /// <summary>
        /// 区分操作类型 
        ...
        private string CheckItem()
        {
            try
            {
                ...
                if (null == deBirth.DateTime || string.IsNullOrEmpty(deBirth.Text))
                {
                    this.deBirth.Focus();
                    return "请选择出生日期";
                }
                ...
                else if (DateTime.Parse(deBirth.DateTime.ToString("yyyy-MM-dd") + " " + teAdmitDate.Time.ToString("HH:mm")) < DateTime.Parse((this.gridControl2.DataSource as DataTable).Select("NOOFINPAT = '" + NOOFINPAT + "'")[0]["ADMITDATE"].ToString()))
                {
                    this.deBirth.Focus();
                    return "出生日期不能小于母亲入院日期";
                }
                return "";
            }
            catch (Exception ex)
        ...
        private PatientEntity SetEntityByPage()
        {
            ...
            //patEntity.Birth = deBirth.DateTime.ToString("yyyy-MM-dd HH:mm:ss");/*deBirth.EditValue.ToString("yyyy-MM-dd");*/
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                patEntity.PatNoOfHis = Guid.NewGuid().ToString().Substring(1, 14);
        ...
        public static string CalcDisplayAge(DateTime birthday, DateTime endDate)
        {
            string displayAge;
            int accAge;
            CalculateAge(birthday, DateTime.Now, out displayAge, out accAge);
            //int ResultAge = Int32.Parse(displayAge+1);
            if (displayAge == "0天")
            {
                displayAge = "1天";
            }
        ...
        public static void CalculateAge(DateTime birthday, DateTime endDate, out string displayAge, out int accurateAge)
        {
            ...
            if (endDate.Day >= birthday.Day)
                dayPart = (endDate.Day - birthday.Day) + 1;//edit by ywk 2012年8月9日14:24:23
        ...
        private void btnADD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.txtAge.Text = CalcDisplayAge(deBirth.DateTime, DateTime.Now);
        ...
        private void SetPatientsBaby_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (m_NurseForm != null)
            {
                m_NurseForm.RefreshPat(EditedPats);
            }
        }
