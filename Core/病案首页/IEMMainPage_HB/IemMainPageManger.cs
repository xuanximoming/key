            ...
            try
            {
                ...
                SqlParameter[] sqlpars = new SqlParameter[]
                {
                    ...
                    new SqlParameter("@result3",SqlDbType.Structured ),
                     new SqlParameter("@result4",SqlDbType.Structured )
                };
                ...
                sqlpars[5].Direction = ParameterDirection.Output;
                DataSet dataSet = DrectSoft.DSSqlHelper.DS_SqlHelper.ExecuteDataSet("IEM_MAIN_PAGE.usp_getieminfo_2012", sqlpars, CommandType.StoredProcedure);
                ...
                if (string.IsNullOrEmpty(IemInfo.IemOthers.IEM_MainPage_Other_NO))
                {
                    GetIemOthers(GetIemmainpageNO(CurrentInpatient.NoOfFirstPage));
                }
...
where t.IEM_MAINPAGE_NO={0} and t.VALID=1", NoOfFirstPage);
        ...
        private void GetIemBasInfo(DataTable dataTable)
        {
            ...
            if (DefaultData.Rows.Count > 0)
            {
                ...
                DefaultInfo.AdmitDate = DefaultData.Rows[0]["INWARDDATE"].ToString() == "" ? DefaultData.Rows[0]["ADMITDATE"].ToString() : DefaultData.Rows[0]["INWARDDATE"].ToString();//入区时间//默认表也取入区时间add by ywk 2013年1月17日13:22:22
            ...
            if (PatData.Rows.Count > 0)
            {
                ...
                //中心医院需求，病案首页的入院时间应为入区时间 edit by ywk 2013年1月17日13:16:38 
                DefaultInfo.AdmitDate = PatData.Rows[0]["INWARDDATE"].ToString() == "" ? DefaultInfo.AdmitDate : PatData.Rows[0]["INWARDDATE"].ToString();
                ...
                //沒有年齡？
                //DefaultInfo.Age=PatData-

                //中心医院不要默认显示这几个医师  edit by ywk 2013年1月18日10:56:16 
                string isshowdefault = GetConfigValueByKey("IsShowDefaultDoc") == "" ? "1" : GetConfigValueByKey("IsShowDefaultDoc");
                if (isshowdefault == "1")
                {
                ...
                {
                    //(除了基本信息还有)科主任，主治和主任医师
                    DefaultInfo.Section_DirectorID = "";
                    DefaultInfo.Section_DirectorName = "";
                    DefaultInfo.DirectorID = "";
                    DefaultInfo.DirectorName = "";
                    DefaultInfo.Vs_EmployeeID = "";
                    DefaultInfo.Vs_EmployeeName = "";
                    DefaultInfo.Resident_EmployeeID = "";
                    DefaultInfo.Resident_EmployeeName = "";
                }
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                IemInfo.IemBasicInfo.AdmitDate = row["INWARDDATE"].ToString() == "" ? row["ADMITDATE"].ToString() : row["INWARDDATE"].ToString();//入区时间
                ...
                #region 对几个详细地址的处理 add by cyq 2012-12-27
                //现住址详细地址
                IemInfo.IemBasicInfo.XZZ_DetailAddress = null != row["xzz_detailaddress"] ? row["xzz_detailaddress"].ToString() : "";
                //户口地址详细地址
                IemInfo.IemBasicInfo.HKDZ_DetailAddress = null != row["hkdz_detailaddress"] ? row["hkdz_detailaddress"].ToString() : "";
                #endregion
            ...
            if (dataTable.Rows.Count == 0)
            {
                ...
                #region 对于几个详细地址的处理 add by cyq 2012-12-27
                IemInfo.IemBasicInfo.XZZ_DetailAddress = string.Empty;
                IemInfo.IemBasicInfo.HKDZ_DetailAddress = string.Empty;
                #endregion
                ...
                {
                    IemInfo.IemBasicInfo.AdmitDate = DtInfoOfAdmission.Rows[0]["InWardDate"].ToString() == "" ? DtInfoOfAdmission.Rows[0]["AdmitDate"].ToString() : DtInfoOfAdmission.Rows[0]["InWardDate"].ToString();//入区时间
                ...
                if (isshowdefault == "0")
                {
                    IemInfo.IemDiagInfo.Vs_EmployeeID = "";//主治医师
                    IemInfo.IemDiagInfo.Resident_EmployeeID = "";//住院医师
                    IemInfo.IemDiagInfo.Section_DirectorID = "";//科主任
                }
                ...
                //}
                //清空初生日期报错调用计算方法没考虑到字段值不存在情况 add by ywk 
            ...
            #endregion
            #region Add by xlb 2013-06-27 计算当前病患年龄和婴儿年龄
            DateTime result;//输出当前病患的出生日期
            //保证当前病患出生日期不为空且可以转换成日期类型变量不是脏数据
            //if (!string.IsNullOrEmpty(IemInfo.IemBasicInfo.Birth.Trim()) && DateTime.TryParse(IemInfo.IemBasicInfo.Birth.Trim(), out  result/*输出参数*/))
            //{
            //    //重新计算年龄避免从inpatient表数据错误也可以正常显示
            //    //這邊重新計算就不能和Inpatient表保持一致!! ADD by ywk
            //    //IemInfo.IemBasicInfo.Age = PatientBasicInfo.CalcDisplayAge(result, DateTime.Now);
            //    int year = DateTime.Now.Year - result.Year;
            //    if (year < 1)
            //    {
            //        IemInfo.IemBasicInfo.Age = PatientBasicInfo.CalcDisplayAge(result, DateTime.Now);
            //        //若岁数小于1岁则自动计算婴儿年龄
            //        IemInfo.IemBasicInfo.MonthAge = PatientBasicInfo.CalcDisplayAge(result, DateTime.Now);
            //    }
            //}
            #endregion
...
select admitdept,admitdate,admitward,admitbed,outwarddate,inwarddate,ATTEND,RESIDENT,CHIEF,dep.name admitdeptname ,w.name admitwardname,u1.name ATTENDName,u2.name RESIDENTName,u3.name CHIEFName,
...
left join ward w1 on i.outhosward=w1.id where noofinpat='{0}'  ", CurrentInpatient.NoOfFirstPage);
        ...
        private void GetItemDiagInfo(DataTable dataTable)
        {
            try
            {
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    dr["iem_mainpage_diagnosis_no"] = row["iem_mainpage_diagnosis_no"].ToString();
                    dr["iem_mainpage_no"] = row["iem_mainpage_no"].ToString();
                    ...
                    dr["MorphoLogyIcd"] = row["MorphoLogyIcd"].ToString();
                    dr["MorphoLogyName"] = row["MorphoLogyName"].ToString();  // add  by jxh  形态学
                ...
                IemInfo.IemDiagInfo.OutDiagTable = DS_BaseService.AddColsDataToDataTable(dt);
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        private void GetItemObsBaby(DataTable dataTable)
        {
            ...
            string config = DS_SqlService.GetConfigValueByKey("ShowObstetricsBabyDept");
        ...
        /// 保存
        /// </summary>
        /// edit by Yanqiao.Cai 2012-12-20 add try ... catch
        /// <param name="m_iemInfo"></param>
        ...
        private void InsertIemInfo_2012()
        {
            ...
            try
            {
                ...
                if (null == iem_mainpage_no || string.IsNullOrEmpty(iem_mainpage_no.Trim()))
                {
                    if (null != IemInfo && null != IemInfo.IemDiagInfo && null != IemInfo.IemDiagInfo.OutDiagTable.Rows && IemInfo.IemDiagInfo.OutDiagTable.Rows.Count > 0)
                    {
                        foreach (DataRow dr in IemInfo.IemDiagInfo.OutDiagTable.Rows)
                        {
                            if (null != dr["iem_mainpage_no"] && !string.IsNullOrEmpty(dr["iem_mainpage_no"].ToString()) && Tool.IsInt(dr["iem_mainpage_no"].ToString().Trim()))
                            {
                                iem_mainpage_no = dr["iem_mainpage_no"].ToString().Trim();
                                break;
                            }
                        }
                    }
                }
                DS_SqlService.DeleteOutHosConditionByPageNo(int.Parse(iem_mainpage_no));
                ...
                foreach (DataRow item in this.IemInfo.IemDiagInfo.OutDiagTable.Rows)
                {
                    ...
                    if (null != item["Diagnosis_Type_Id"] && !string.IsNullOrEmpty(item["Diagnosis_Type_Id"].ToString().Trim()) && item["Diagnosis_Type_Id"].ToString().Trim() != "13")
                    {
                        //获取插入的诊断的ID
                        int iem_mainpage_diagnosis_no = GetDiagIdByItem(item, int.Parse(iem_mainpage_no));
                        //插入出院情况 add by cyq 2012-12-25
                        InsertOutHosCondition(item, iem_mainpage_diagnosis_no, int.Parse(iem_mainpage_no));
                    }
                }
                foreach (DataRow item in this.IemInfo.IemOperInfo.Operation_Table.Rows)
                    InserIemOperInfo_2012(item, m_app.SqlHelper);

                //如果没有产科的执行下面会报错的！！！！！！！add by ywk 2013年1月18日14:03:33 
        ...
        /// <summary>
        /// 根据除iem_mainpage_diagnosis_no之外的其它列来匹配查找出诊断表中iem_mainpage_diagnosis_no
        /// 注：针对诊断的先删除后插入，定位ID问题
        /// </summary>
        /// <param name="drow"></param>
        /// <returns></returns>
        private int GetDiagIdByItem(DataRow drow, int mainpageNo)
        {
            try
            {
                int iem_mainpage_diagnosis_no = (null != drow["iem_mainpage_diagnosis_no"] && !string.IsNullOrEmpty(drow["iem_mainpage_diagnosis_no"].ToString().Trim())) ? int.Parse(drow["iem_mainpage_diagnosis_no"].ToString().Trim()) : -1;
                if (null != drow)
                {
                    string sqlStr = "select * from iem_mainpage_diagnosis_2012 where valide=1 and diagnosis_code=@diagnosisCode ";

                    List<DbParameter> sqlParams = new List<DbParameter>();
                    SqlParameter param1 = new SqlParameter("@diagnosisCode", SqlDbType.Char);
                    param1.Value = null == drow["diagnosis_code"] ? "" : drow["diagnosis_code"].ToString().Trim();
                    sqlParams.Add(param1);

                    if (null != drow["status_id"] && !string.IsNullOrEmpty(drow["status_id"].ToString().Trim()))
                    {
                        sqlStr += " and status_id=@statusID ";
                        SqlParameter param2 = new SqlParameter("@statusID", SqlDbType.Int);
                        param2.Value = (null != drow["status_id"] && !string.IsNullOrEmpty(drow["status_id"].ToString().Trim()) && Tool.IsInt(drow["status_id"].ToString().Trim())) ? int.Parse(drow["status_id"].ToString().Trim()) : -1;
                        sqlParams.Add(param2);
                    }
                    if (null != drow["admitinfo"] && !string.IsNullOrEmpty(drow["admitinfo"].ToString().Trim()))
                    {
                        sqlStr += " and admitinfo=@admitinfo ";
                        SqlParameter param3 = new SqlParameter("@admitinfo", SqlDbType.Char);
                        param3.Value = null == drow["admitinfo"] ? "" : drow["admitinfo"].ToString().Trim();
                        sqlParams.Add(param3);
                    }
                    if (null != drow["IEM_MAINPAGE_NO"] && !string.IsNullOrEmpty(drow["IEM_MAINPAGE_NO"].ToString().Trim()))
                    {
                        sqlStr += " and IEM_MAINPAGE_NO=@mainpageNo ";
                        SqlParameter param4 = new SqlParameter("@mainpageNo", SqlDbType.Int);
                        param4.Value = mainpageNo;
                        sqlParams.Add(param4);
                    }
                    sqlStr += " order by iem_mainpage_diagnosis_no ";

                    DS_SqlHelper.CreateSqlHelper();
                    DataTable dt = DS_SqlHelper.ExecuteDataTable(sqlStr, sqlParams, CommandType.Text);
                    if (null != dt && dt.Rows.Count > 0)
                    {
                        iem_mainpage_diagnosis_no = int.Parse(dt.Rows[0]["iem_mainpage_diagnosis_no"].ToString());
                    }
                }
                return iem_mainpage_diagnosis_no;
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        /// <param name="drow">诊断信息</param>
        private void InsertOutHosCondition(DataRow drow, int diagnosisNo, int mainpageNo)
        {
            try
            {
                if (null == drow)
                ...
                string diagnosis_code = null == drow["diagnosis_code"] ? "" : drow["diagnosis_code"].ToString();
                string diagnosis_name = null == drow["diagnosis_name"] ? "" : drow["diagnosis_name"].ToString();
                int status_id = (null != drow["Status_Id_Out"] && !string.IsNullOrEmpty(drow["Status_Id_Out"].ToString().Trim())) ? int.Parse(drow["Status_Id_Out"].ToString().Trim()) : -1;
                string status_name = null == drow["Status_Name_Out"] ? "" : drow["Status_Name_Out"].ToString();
                string create_user = DS_Common.currentUser.Id;
                string create_time = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                DS_SqlService.InsertOutHosCondition(diagnosisNo, mainpageNo, diagnosis_code, diagnosis_name, status_id, status_name, create_user, create_time);
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        private void EditIemFeeInfo_2012(IemMainPageInfo iemInfo, string type)
        {
            ...
            paraSSYYCXYYCLFFee.Value = iemInfo.IemFeeInfo.SSYYCXCLF;// 耗材类 手术用一次性医用材料费add by ywk 原来重复的了
        ...
        private void EditIemBasicInfo_2012(IemMainPageInfo info, string type)
        {
            try
            {
                ...
                if (info.IemBasicInfo.Birth == null)//add by ywk 2013年5月29日 15:55:08
                {
                    paraBirth.Value = "";
                }
                else
                ...
                paraLATERHOSCOMAHOUR.Value = info.IemBasicInfo.LaterHosComaHour;//原来的LaterHosComaDay
        ...
        private void InserIemDiagnoseInfo_2012(DataRow info, IDataAccess sqlHelper, Iem_Mainpage_Diagnosis Iemm)
        {
            ...
            SqlParameter paraMorphoLogyIcd = new SqlParameter("@MorphoLogyIcd", SqlDbType.VarChar, 60);
            paraMorphoLogyIcd.Value = info["MorphoLogyIcd"];
            SqlParameter paraMorphoLogyName = new SqlParameter("@MorphoLogyName", SqlDbType.VarChar, 100);
            paraMorphoLogyName.Value = info["MorphoLogyName"];  // add jxh
            ...
            SqlParameter[] paraColl = new SqlParameter[] { paraIem_Mainpage_NO, paraDiagnosis_Type_Id, paraDiagnosis_Code, paraDiagnosis_Name,paraMorphoLogyIcd,paraMorphoLogyName, paraStatus_Id, paraOrder_Value, paraCreate_User ,
            ...
            sqlHelper.ExecuteNoneQuery("IEM_MAIN_PAGE.usp_edif_iem_mainpage_diag_hb", paraColl, CommandType.StoredProcedure);
        }
        ...
        private void UpdateItemInfo_2012()
        {
            try
            {
                ...
                // 先把之前的诊断出院情况，都给CANCLE add by cyq 2012-12-25
                string iem_mainpage_no = IemInfo.IemBasicInfo.Iem_Mainpage_NO;
                if (null == iem_mainpage_no || string.IsNullOrEmpty(iem_mainpage_no.Trim()))
                {
                    if (null != IemInfo && null != IemInfo.IemDiagInfo && null != IemInfo.IemDiagInfo.OutDiagTable.Rows && IemInfo.IemDiagInfo.OutDiagTable.Rows.Count > 0)
...
where IEM_MAINPAGE_NO='{10}'",
        ...
        private void UpdateInPaient(IemMainPageInfo info, string isupdateIdNO, IDataAccess sqlHelper)
        {
            ...
            if (info.IemBasicInfo.Birth == null || info.IemBasicInfo.Birth == "")//add by ywk 二〇一三年五月二十九日 15:22:44 
            {
                paraAge.Value = 0;
            }
            else
            ...
            #region 新增的两个几个详细地址(现详细住址、户口详细地址)
            SqlParameter paraXZZDetail = new SqlParameter("@XZZDetailAddr", SqlDbType.VarChar, 300);
            paraXZZDetail.Value = info.IemBasicInfo.XZZ_DetailAddress;//现住址-详细住址
            SqlParameter paraHKDZDetail = new SqlParameter("@HKDZDetailAddr", SqlDbType.VarChar, 300);
            paraHKDZDetail.Value = info.IemBasicInfo.HKDZ_DetailAddress;//户口地址-详细详细
            #endregion
            ...
            paraOfficePlace,
                ...
                ,paraIsUpdateIDNO,paracsdaddress,parajgaddress,paraxzzdaddress,parahkdzaddress,paraXZZDetail,paraHKDZDetail};
            try
            {
                //if (sqlHelper == null)
