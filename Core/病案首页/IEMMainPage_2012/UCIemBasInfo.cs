...
namespace DrectSoft.Core.IEMMainPage
{
    public partial class UCIemBasInfo : UserControl
    {
        ...
        void deOutWardDate_EditValueChanged(object sender, EventArgs e)
        {
            ...
            DateTime dateBegin = DateTime.Parse(deAdmitDate.DateTime.ToString("yyyy-MM-dd") + " " + teAdmitDate.EditValue.ToString());
            DateTime dateEnd = DateTime.Parse(deOutWardDate.DateTime.ToString("yyyy-MM-dd") + " " + teOutWardDate.EditValue);
        ...
        private void FillUIInner()
        {
            try
            {
                ...
                {
                    currentPatientInfo = YD_SqlService.GetPatientInfo(info.IemBasicInfo.NoOfInpat);
                }
                else
                {
                ...
                //第二行
                //Modify by xlb 2013-03-23
                string month = string.Empty;
                string day = string.Empty;
                string temp = string.Empty;
                if (info.IemBasicInfo.MonthAge.Contains("月"))
                {
                    if (info.IemBasicInfo.MonthAge.Contains("个"))
                    {
                        month = info.IemBasicInfo.MonthAge.Split('个')[0];
                        int index = info.IemBasicInfo.MonthAge.IndexOf("月");
                        temp = info.IemBasicInfo.MonthAge.Substring(index + 1, info.IemBasicInfo.MonthAge.Length - index - 1);
                    }
                    else
                    {
                        month = info.IemBasicInfo.MonthAge.Split('月')[0];
                        int index = info.IemBasicInfo.MonthAge.IndexOf("月");
                        temp = info.IemBasicInfo.MonthAge.Substring(index + 1, info.IemBasicInfo.MonthAge.Length - index - 1);
                    }
                }
                if (info.IemBasicInfo.MonthAge.Contains('天'))
                {
                    if (string.IsNullOrEmpty(temp))
                    {
                        day = info.IemBasicInfo.MonthAge.Split('天')[0];
                    }
                    else
                    {
                        day = temp.Split('天')[0];
                ...
                if (!info.IemBasicInfo.MonthAge.Contains("月") && !info.IemBasicInfo.MonthAge.Contains("天"))
                {
                    if (string.IsNullOrEmpty(info.IemBasicInfo.MonthAge) && isNeedAuto/*对于新增的记录则计算*/)
                    {
                        //TimeSpan XseAge=(TimeSpan)(DateTime.Now-deBirth.DateTime);
                        int times = Convert.ToInt32((DateTime.Now - deBirth.DateTime).TotalDays);
                        if (times > 365)
                        {
                            month = info.IemBasicInfo.MonthAge;
                            day = info.IemBasicInfo.MonthAge;
                        }
                        else
                        {
                            month = Convert.ToString(times / 30);
                            day = Convert.ToString(times % 30);
                        }
                        //TimeSpan timeSpan = new TimeSpan(0, 0, 0, Convert.ToInt32(XseAge));
                    }
                    else
                    {
                        month = info.IemBasicInfo.MonthAge;
        ...
        private void GetUI()
        {
            try
            {
                ...
                {
                    m_IemInfo.IemBasicInfo.OutWardDate = "";
                }
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (editFlag)
                {
                    ...
                    //int num = YD_SqlService.SetRecordsRebacked(CurrentInpatient.NoOfFirstPage.ToString());
                    //if (num == 0)
                    //{
