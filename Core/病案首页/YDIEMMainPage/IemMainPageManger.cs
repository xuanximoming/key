...
namespace DrectSoft.Core.IEMMainPage
{
    public class IemMainPageManger
    {
        ...
        public bool SetExetionButton()
        {
            try
            {
                string sql = "select count(*) from iem_mainpage_except where valide='1'";/*扩展维护表sql*/
                DS_SqlHelper.CreateSqlHelper();
                DataTable dtCount = DS_SqlHelper.ExecuteDataTable(sql, CommandType.Text);
...
where t.IEM_MAINPAGE_NO={0} and t.VALID=1", NoOfFirstPage);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    IemInfo.IemOthers.OtherIEM_MainPage_NO = dt.Rows[0]["IEM_MAINPAGE_NO"].ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void GetIemBasInfo(DataTable dataTable)
        {
            try
            {
                ...
                {
                    DefaultInfo.PatNoOfHis = DefaultData.Rows[0]["noofrecord"].ToString();
                ...
                if (PatData.Rows.Count > 0)
                {
                    ...
                    DefaultInfo.NoofReocrd = PatData.Rows[0]["noofrecord"].ToString() == "" ? DefaultInfo.NoofReocrd : PatData.Rows[0]["noofrecord"].ToString();
                ...
                if (dataTable.Rows.Count == 0)
                {
                    ...
                    IemInfo.IemBasicInfo.NoofRecord = DefaultInfo.NoofReocrd;
                ...
                {
                    IemInfo.IemBasicInfo.NoofRecord = GetPatData(IemInfo.IemBasicInfo.Mother).Rows[0]["noofrecord"].ToString();
                }
            }
            catch (Exception ex)
            {
...
left join ward w1 on i.outhosward=w1.id where noofinpat='{0}'  ", CurrentInpatient.NoOfFirstPage);
        ...
        private void GetItemObsBaby(DataTable dataTable)
        {
            try
            {
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    if (index == 0)
                    {
                        IemInfo.IemObstetricsBaby.BabyIEM_MainPage_NO = row["Iem_Mainpage_NO"].ToString();
                        ...
                        IemInfo.IemObstetricsBaby.BabyWeight = row["Weight"].ToString();
        ...
        private void EditIemBasicInfo_2012(IemMainPageInfo info, string type)
        {
            try
            {
                ...
                paraLATERHOSCOMAHOUR.Value = info.IemBasicInfo.LaterHosComaHour;//edit by ywk 
        ...
        private void InsertIemObstetricsBaby(Iem_MainPage_ObstetricsBaby baby, IDataAccess sqlHelper)
        {
            try
            {
                baby.BabyCreate_User = m_app.User.DoctorId;
                baby.BabyIEM_MainPage_NO = this.IemInfo.IemBasicInfo.Iem_Mainpage_NO.ToString();
                SqlParameter paraIem_Mainpage_NO = new SqlParameter("@iem_mainpage_no", SqlDbType.Decimal);
                paraIem_Mainpage_NO.Value = baby.BabyIEM_MainPage_NO;
                SqlParameter paraID = new SqlParameter("@IBSBABYID", SqlDbType.VarChar, 1);
                paraID.Value = baby.IBSBABYID;
                SqlParameter paraTC = new SqlParameter("@TC", SqlDbType.VarChar, 1);
                paraTC.Value = baby.TC;
                ...
                paraHeigh.Value = baby.Heigh;
                SqlParameter paraWeight = new SqlParameter("@Weight", SqlDbType.VarChar, 10);
                paraWeight.Value = baby.BabyWeight;
                SqlParameter paraCCQK = new SqlParameter("@CCQK", SqlDbType.VarChar, 1);
                paraCCQK.Value = baby.CCQK;
                SqlParameter paraBITHDAY = new SqlParameter("@BITHDAY", SqlDbType.VarChar, 1);
                paraBITHDAY.Value = baby.BithDay;
                ...
                paraCYQK.Value = baby.CYQK;
                SqlParameter paraCreate_User = new SqlParameter("@Create_User", SqlDbType.VarChar, 10);
                paraCreate_User.Value = baby.BabyCreate_User;
...
where IEM_MAINPAGE_NO='{10}'",
        ...
        private void UpdateInPaient(IemMainPageInfo info, string isupdateIdNO, IDataAccess sqlHelper)
        {
            ...
            SqlParameter paraAge = new SqlParameter("@Age", SqlDbType.VarChar, 10);
            // paraAge.Value = info.IemBasicInfo.Age;//年龄 edit by ywk
            //paraAge.Value = info.IemBasicInfo.Age;//
            if (!string.IsNullOrEmpty(info.IemBasicInfo.Birth.Trim()))
            {
                paraAge.Value = DateTime.Parse(info.IemBasicInfo.AdmitDate).Year - DateTime.Parse(info.IemBasicInfo.Birth).Year;//若birth为空呢？
            }
            else
            {
                paraAge.Value = DBNull.Value;
            }
