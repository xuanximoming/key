...
namespace DrectSoft.Core.MedicalRecordQuery
{
    ...
    /// 创建日期：2012-12-28
    /// </summary>
    public partial class SetAttendDoctorForm : DevBaseForm
    {
        IEmrHost m_app;
        ...
        public SetAttendDoctorForm(IEmrHost App)
        {
            InitializeComponent();
            m_app = App;
        }
        ...
        private void btn_saveRights_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (DrectSoft.Common.Ctrs.DLG.MyMessageBox.Show("您没有勾选组合权限，确定要保存吗？", "保存组合权限", DrectSoft.Common.Ctrs.DLG.MyMessageBoxButtons.OkCancel) == DialogResult.Cancel)
                    {
                        return;
                    }
                }
