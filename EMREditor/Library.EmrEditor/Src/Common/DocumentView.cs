...
namespace DrectSoft.Library.EmrEditor.Src.Common
{
    ...
    public class DocumentView : System.IDisposable
    {
        ...
        /// <summary>
        /// 因为新插入字符时，使用的是默认字体，大小，所以没有给它单独设置这些属性，
        /// </summary>
        ...
        public void DrawSelectionFrame(System.Drawing.Rectangle myBounds)
        {
            if (myGraph != null)
                System.Windows.Forms.ControlPaint.DrawSelectionFrame
                    (myGraph, false, myBounds, System.Drawing.Rectangle.Empty, System.Drawing.SystemColors.ButtonFace);
            ...
            if (myGraph != null)
            {
                if (WeiWenProcess.weiwen)
                {
                    ...
                    for (int iCount = 0; iCount < iPos.Length; iCount += 4)
                        this.myGraph.DrawLine(myPen, x + iPos[iCount], y + iPos[iCount + 1], x + iPos[iCount + 2], y + iPos[iCount + 3]);
                    #region 再画实心三角形
        ...
        public int DefaultRowPixelHeight
        {
            get
            {
                return (int)myDefaultFont.GetHeight();
            }
        }
        /// <summary>
        /// 数据域背景样式
