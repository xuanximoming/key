...
{
    public partial class UCEmrInputout : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public UCEmrInputout(Inpatient inpatient, IEmrHost app)
        {
            try
            {
                m_WaitDialog = new WaitDialogForm("正在创建病历编辑组件...", "请稍候");
        ...
        public UCEmrInputout(Inpatient inpatient, IEmrHost app, FloderState floadState)
        {
            try
            {
                m_WaitDialog = new WaitDialogForm("正在创建病历编辑组件...", "请稍候");
        ...
        /// <param name="e"></param>
        private void UCEmrInputout_Load(object sender, EventArgs e)
        {
            try
            {
                OnLoad();
        ...
        public void OnLoad()
        {
            try
            {
                ...
                if (floaderState == FloderState.All || floaderState == FloderState.Default || floaderState == FloderState.Doctor || floaderState == FloderState.NoneAudit)
                {
                    ...
                    ///历史病历自动导入
                    this.FindForm().Shown += new EventHandler(UCEmrInputout_Shown);
                }
        ...
        private void AddEmrInputBody()
        {
            try
            {
                ...
                {
                    UCEmrInputBody body = new UCEmrInputBody(m_CurrentInpatient, m_app, floaderState, 1);
        ...
        void UCEmrInputout_Shown(object sender, EventArgs e)
        {
            try
            {
                //自动显示病历批量导入界面
        ...
        void UCEmrInputout_Activated(object sender, EventArgs e)
        {
            try
            {
                //自动显示病历批量导入界面
                CurrentInputBody.AutoShowHistoryEmrBatchInForm();
                this.FindForm().Activated -= new EventHandler(UCEmrInputout_Activated);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UCEmrInputout_SizeChanged(object sender, EventArgs e)
        {
            try
            {
