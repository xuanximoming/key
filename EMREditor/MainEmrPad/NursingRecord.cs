...
using Yidansoft.Core.MainEmrPad.ThreeLevelCheck;
using DevExpress.XtraEditors;
using DevExpress.XtraEditors.Controls;
using Yidansoft.Core.MainEmrPad.HelpForm;
using DevExpress.XtraGrid.Views.Grid;
using DevExpress.XtraNavBar;
using Yidansoft.Core.MainEmrPad.HistoryEMR;
using System.Runtime.InteropServices;
using YidanSoft.Core.IEMMainPage;
//using YidanSoft.Core.IEMMainPageBase;
using System.Data.SqlClient;
using YiDanSoft.Core.CommonTableConfig.CommonNoteUse;
using YiDanSoft.Core.CommonTableConfig;
...
namespace Yidansoft.Core.MainEmrPad
{
    public partial class UCEmrInput : DevExpress.XtraEditors.XtraUserControl //, IStartPlugIn
    {
        ...
        private Employee CurrentEmployee
        {
            get
            {
                ...
                {
                    _currentEmployee = new YidanSoft.Common.Eop.Employee(m_app.User.Id);
        ...
        public void SetInnerVar(IYidanEmrHost app, RecordDal recordDal)
        {
            try
            {
                m_app = app;
        ...
        private void LoadModelFromTree(TreeListNode node)
        {
            try
            {
                    ...
                    else
                    {
                        ...
                        if (container.EmrContainerType == ContainerType.None)
                        {
                            if (m_TempContainerPages.ContainsKey(container))
                            {
                                ...
                                if (ctl != null && ctl.GetType().FullName.Equals("YidanSoft.Core.YDNurseDocument.MainNursingMeasure"))//如果是新的体温单界面
                                {
                                    (ctl as YidanSoft.Core.YDNurseDocument.MainNursingMeasure).simpleButtonRefresh_Click(null, null);
                                }
                            }
                            else
                            {
                ...
                if (node.Tag is InCommonNoteEnmtity)
                {
                    ...
                    if (tabpage == null)
                    {
                        CommonNoteBiz commonNoteBiz = new YiDanSoft.Core.CommonTableConfig.CommonNoteBiz(m_app);
                ...
                string isopenOwnerSign = YD_SqlService.GetConfigValueByKey("IsOpenOwnerSign") == "" ? "0" : YD_SqlService.GetConfigValueByKey("IsOpenOwnerSign");
                ...
                for (int i = 0; i < emrModelList.Count - 1; i++)
                {
                    ...
                    if (dtChild >= dtmin && dtChild <= dtmax)  //新增时间在相邻2个时间之间
                    {
                        if (emrModelChild.DepartCode == emrModelList[i].DepartCode
                            && emrModelChild.DepartCode == emrModelList[i + 1].DepartCode)  //加入项目 与相邻2个科室相同
                        {
                            int firstindex = 0;  //同一科室的第一个
                            for (int j = i; j > 0; j--)
                            {
                                if (emrModelList[j].DepartCode != emrModelList[j - 1].DepartCode)  //向上循环 获取相邻2个科室不同的序号
                                {
                                    firstindex = j;
                                    break;
                                }
                           ...
                           && emrModelChild.DepartCode != emrModelList[i + 1].DepartCode)  //科室与上一个项目相同
                        {
                            for (int j = i; j >= 0; j--)
                            {//向上循环
                           ...
                           && emrModelChild.DepartCode == emrModelList[i + 1].DepartCode)  //科室与下一个项目相同
                        {
                            for (int j = i + 1; j < emrModelList.Count; j++)
                            {//向下循环
                                if (emrModelList[j].DepartCode == emrModelChild.DepartCode
        ...
        private void CheckIsAllowEdit(EmrModel model)
        {
            try
            {
                ...
                if (floaderState == FloderState.None || floaderState == FloderState.FisrtPage)
                {
                    SetDocmentReadOnlyMode();
                }
            }
        ...
        private void SetTableCellProperty()
        {
            ...
            //else
            //{
            //    MessageBox.Show("光标必须在表格中才能更改单元格的属性", "不能打开单元格属性", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //}
        }
        ...
        private void SetLockImage(DevExpress.XtraTreeList.GetStateImageEventArgs e)
        {
            try
            {
                ...
                else if (floaderState == FloderState.FisrtPage)
                {//病案管理 - 未归档的出院病人列表
        ...
        private void SaveDocment(EmrModel model)
        {
            ...
            //m_IsSaving = true;
            //edit by Yanqiao.Cai 2012-12-03 此变量改为全局变量
            //bool isSaved = true;//病程是否保存成功
            try
            {
                ...
                {
                    if (m_app.CustomMessageBox.MessageShow(string.Format("该患者为男性，病历内容含有月经史，您是否要继续保存？"), CustomMessageBoxKind.QuestionOkCancel) == DialogResult.OK)
                    {
                        Save(model);
                        this.HideWaitDialog();
                    }
        ...
        private void Save(EmrModel model)
        {
            try
            {
                ...
                if (model.DailyEmrModel)
                {
                    ...
                    {
                        #region 注释 edit by cyq 2012-11-22
                        ...
                        DataTable allrecords = YD_SqlService.GetRecordsByNoofinpatContainDel(int.Parse(m_CurrentInpatient.NoOfFirstPage.ToString()));
                        ...
                        if (firstEmr_ID == -1)
                        {//新增首程
                            if (ExistTheFirstRecord(firstEmr, m_CurrentInpatient.NoOfFirstPage.ToString()))
                            {//首程已存在
                                //add by Yanqiao.Cai 2012-12-03 此变量改为全局变量
                                isSaved = false;
                                if (YiDanCommon.Ctrs.DLG.YiDanMessageBox.Show("该病人已存在首程，不可重复创建。您是否要刷新病程记录？", "首程已存在", YiDanCommon.Ctrs.DLG.YiDanMessageBoxButtons.YesNo) == DialogResult.No)
                                {
                                    return;
                                }
                                //刷新病历树节点
                        ...
                        //保存节点
                        foreach (EmrModel em in list.Where(p => p.InstanceId != firstEmr.InstanceId))
                        {
            ...
            try
            {
                if (null != emrList && emrList.Count > 0)
                {
                    ...
                    foreach (EmrModel model in firstEmrList)
                    {
                        ...
                        if (firstEmr_ID == -1)
                        {//新增首程
                            if (ExistTheFirstRecord(model, m_CurrentInpatient.NoOfFirstPage.ToString()))
                            {//首程已存在
                                //add by Yanqiao.Cai 2012-12-03 此变量改为全局变量
                                isSaved = false;
                                if (YiDanCommon.Ctrs.DLG.YiDanMessageBox.Show("该病人已存在首程，不可重复创建。您是否要刷新病程记录？", "首程已存在", YiDanCommon.Ctrs.DLG.YiDanMessageBoxButtons.YesNo) == DialogResult.No)
        ...
        /// <param name="noofinpat">住院号</param>
        /// <returns></returns>
        private bool ExistTheFirstRecord(EmrModel model, string noofinpat)
        {
            try
            {
        ...
        private void AddEmrModel(TreeListNode node)
        {
            ...
            //{
            //    YiDanCommon.Ctrs.DLG.YiDanMessageBox.Show("对不起，三级检诊功能和设置个人修改痕迹不可同时开放");
            //    return;
            //}
            ...
            //    if (YiDanMessageBox.Show("您确认此时并开启留痕吗？", "提示", YiDanMessageBoxButtons.YesNo) == DialogResult.Yes)
            //    {
            //        this.CurrentForm.zyEditorControl1.EMRDoc.Info.DocumentModel = DocumentModel.Edit;
            //        this.CurrentForm.zyEditorControl1.EMRDoc.SaveLogs.CurrentSaveLog.Level = 1;         //这里传入用户级别
        ...
        private bool ProcessDailyTemplateForm(EmrModel model, TreeListNode node)
        {
            ...
            using (DailyTemplateForm daily = new DailyTemplateForm(dt, titleName, false,m_app.CurrentPatientInfo.NoOfFirstPage.ToString()))
            {
        ...
        private void CheckDocBYConfigItem()
        {
            ...
            if (m_CurrentModel == null)
            {
                m_app.CustomMessageBox.MessageShow("病历内容不能为空");
                return;
            }
            else
            {
        ...
        private void ResetEditModeAction(EmrModelContainer container)
        {
            ...
            {
                m_app.CustomMessageBox.MessageShow("ResetEditModeAction方法出错" + ex.Message);
            }
        ...
        const string sql_queryTree = "select * from emrtemplet_item_person_catalog where (deptid='{0}' and isperson='0') or (isperson='1' and createusers='{1}')   and deptid='{0}' or createusers='{1}' order by NAME ";
        const string sql_queryLeaf = "select * from emrtemplet_item_person a where (a.deptid='{0}' and isperson=0) or (a.isperson = 1 and a.createusers = '{1}')   and deptid='{0}' or createusers='{1}' order by NAME ";
        #region "暂停"
        ...
        private void barButtonItemEmrPrint_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (m_CurrentModel != null)
            {
                ...
                if (CurrentForm.zyEditorControl1.EMRDoc.Modified && barButtonSave.Enabled == true)
                {
                    if (m_app.CustomMessageBox.MessageShow(" 该病历已修改，需要保存后再打印，您是否要保存？", YidanSoft.Core.CustomMessageBoxKind.QuestionYesNo) == System.Windows.Forms.DialogResult.Yes)
                    {
                        SaveDocment(m_CurrentModel);
                    }
                    else
        ...
        public void PatientChangedByIYidanEmrHost(decimal noofinpat)
        {
            CloseAllTabPages();
    ...
    */
        ...
        /// 病案首页病患信息
        /// </summary>
        public YidanSoft.Core.IEMMainPage.IemMainPageInfo IemInfo
        {
            get;
            set;
        }
        ...
        private void CheckInpatient(string noofinpat, out string TipMessage)
        {
            ...
            {
                if (m_app.CustomMessageBox.MessageShow(string.Format("诊断符合情况有输入[误诊]的选项，您是否要继续？"), CustomMessageBoxKind.QuestionOkCancel) == DialogResult.OK)
                {
                    CheckRight = true;
                }
                else
        ...
        private void btnCallPacs_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            else if (valuestr == "yczxyy")  //宜昌中心医院
            {
                PacsZhongXinHos();
            }
            else
            {
                m_app.CustomMessageBox.MessageShow("没有配置pacs调用");
            }
        ...
        private void PacsWHSS()
        {
            ...
            if (hasFile)
            {
                ...
                string infostr = string.Format(@"G_study.inhospitalno='{0}'", m_app.CurrentPatientInfo.NoOfHisFirstPage);
                Process.Start(fileName, infostr);
            }
            else
            {
                m_app.CustomMessageBox.MessageShow("Pacs路径不存在");
            }
        }
        ...
        private void btnAddCommonRecord_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (diaresult == DialogResult.OK)
            {
                ...
                InCommonNoteBiz icombiz = new YiDanSoft.Core.CommonTableConfig.CommonNoteUse.InCommonNoteBiz(m_app);
        ...
        private void btnDelCommonNote_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            DialogResult diaresult = m_app.CustomMessageBox.MessageShow("您确定要删除该记录吗？", CustomMessageBoxKind.QuestionYesNo);
        ...
        /// </summary>
        private void RemoveAllPagesExceptMain()
        {
            try
            {
                if (xtraTabControl1.TabPages.Count > 0)
                {
                    if (xtraTabControl1.TabPages.Any(p => p == xtraTabPageIEMMainPage))
                    {
                        xtraTabControl1.TabPages.Clear();
                        xtraTabControl1.TabPages.Add(xtraTabPageIEMMainPage);
                    }
                    else
                    {
                        xtraTabControl1.TabPages.Clear();
        ...
        private void SetSelectNode(string nodeName)
        {
            try
            {
                ...
                TreeListNodes nodes = treeList1.Nodes;
