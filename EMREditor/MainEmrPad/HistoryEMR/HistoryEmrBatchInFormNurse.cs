...
{
    /// <summary>
    /// 历史病历批量导入 Add by cyq 2013-06-04
    /// </summary>
    public partial class HistoryEmrBatchInFormNurse : DevBaseForm
    {
        ...
        private void InitDataHistory(string dateBegin, string dateEnd)
        {
            try
            {
                ...
                DataTable dt = DS_SqlService.GetHistoryInpatients((int)m_CurrentInpatient.NoOfFirstPage, 1, dtBegin, dtEnd);
        ...
        private void InitEmrTreeNodes()
        {
            try
            {
                ...
                if (drv != null)
                {
                    int noofinpat = Convert.ToInt32(drv["noofinpat"]);
                    DataTable dt = m_HistoryEmrBll.GetNurseEmrListByNoofinpat(noofinpat);
