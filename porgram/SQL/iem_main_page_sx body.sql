    ...
    IF v_edittype = '1' THEN
      ...
      insert into iem_mainpage_basicinfo_sx
        (IEM_MAINPAGE_NO, ---- '病案首页标识';
         ...
         PATHOLOGY_OBSERVATION_FQ,-----病理分期：
         FOLLOW_UP_CYCLE,----随诊期限
         ALLERGIC_DRUG, ---- '过敏药物';
         ...
         INHOSINFO,-----入院情况 □ 1. 危 2. 急 3. 一般
         INHOSCALL,----住院期间是否告病危或病重 □ 1. 是 2. 否
         TYPEHOSPITAL,----其他医疗机构
         OUTHOSTYPE, ---- '离院方式 □ 1.医嘱离院  2.医嘱转院 3.医嘱转社区卫生服务机构/乡镇卫生院 4.非医嘱离院5.死亡9.其他';
         ANTIBACTERIAL_DRUGS,----Ⅰ类手术切口预防性应用抗菌药物
         DURATIONDATE,---使用持续时间
         COMBINED_MEDICATION,----联合用药
         
         PATHWAY_FLAG,----联合用药
         PATHWAY_OVER,----联合用药
         PATH_OUT_REASON,----联合用药
         VARIATION_FLAG,----联合用药
         VARIATION_REASON,----联合用药
         REHOSPITALIZATION,----联合用药
         INTERVALDATE,----联合用药
      ...
      values
        (seq_iem_mainpage_basicinfo_id.NEXTVAL, ---- '病案首页标识';
         ...
         v_INHOSINFO,------入院情况 □ 1. 危 2. 急 3. 一般
         v_INHOSCALL,---住院期间是否告病危或病重 □ 1. 是 2. 否
         v_TYPEHOSPITAL, ----其他医疗机构
         v_OUTHOSTYPE, ---- '离院方式 □ 1.医嘱离院  2.医嘱转院 3.医嘱转社区卫生服务机构/乡镇卫生院 4.非医嘱离院5.死亡9.其他';
         v_ANTIBACTERIAL_DRUGS,----Ⅰ类手术切口预防性应用抗菌药物
         v_DURATIONDATE,---使用持续时间
         v_COMBINED_MEDICATION,----联合用药
         
         v_PATHWAY_FLAG,----联合用药
         v_PATHWAY_OVER,----联合用药
         v_PATH_OUT_REASON,----联合用药
         v_VARIATION_FLAG,----联合用药
         v_VARIATION_REASON,----联合用药
         v_REHOSPITALIZATION,----联合用药
         v_INTERVALDATE,----联合用药
    ...
    ELSIF v_edittype = '2' THEN
      ...
      update iem_mainpage_basicinfo_sx
         set PATNOOFHIS               = v_PATNOOFHIS, --病案号
             ...
             MAINDIAGDATE             = v_MAINDIAGDATE,----主要诊断确诊日期
             ...
             PATHOLOGY_OBSERVATION_FQ = v_PATHOLOGY_OBSERVATION_FQ,-----病理分期：,-----病理分期：
             FOLLOW_UP_CYCLE          = v_FOLLOW_UP_CYCLE,----随诊期限
             ...
             ATTENDING_PHYSICIAN_CODE = v_ATTENDING_PHYSICIAN_CODE,---主诊医师,---主诊医师
             ...
             INHOSINFO               = v_INHOSINFO,-----入院情况 □ 1. 危 2. 急 3. 一般
             INHOSCALL               = v_INHOSCALL,----住院期间是否告病危或病重 □ 1. 是 2. 否
             TYPEHOSPITAL            = v_TYPEHOSPITAL,----其他医疗机构
             OUTHOSTYPE              = v_OUTHOSTYPE, --离院方式 □ 1.医嘱离院  2.医嘱转院 3.医嘱转社区卫生服务机构/乡镇卫生院 4.非医嘱离院5.死亡9.其他
             ANTIBACTERIAL_DRUGS     = v_ANTIBACTERIAL_DRUGS,----Ⅰ类手术切口预防性应用抗菌药物
             DURATIONDATE            = v_DURATIONDATE,---使用持续时间
             COMBINED_MEDICATION     = v_COMBINED_MEDICATION,----联合用药
             
             PATHWAY_FLAG          = v_PATHWAY_FLAG,----联合用药
             PATHWAY_OVER          = v_PATHWAY_OVER,----联合用药
             PATH_OUT_REASON       = v_PATH_OUT_REASON,----联合用药
             VARIATION_FLAG        = v_VARIATION_FLAG,----联合用药
             VARIATION_REASON      = v_VARIATION_REASON,----联合用药
             REHOSPITALIZATION     = v_REHOSPITALIZATION,----联合用药
             INTERVALDATE          = v_INTERVALDATE,----联合用药
             ...
             FOLLOW_UP               = v_FOLLOW_UP,----随诊 □ 1. 是 2. 否,
             ...
             RYXYZD_NAME  = v_RYXYZD_NAME, --入院诊断
             RYXYZD_CODE  = v_RYXYZD_CODE,--入院诊断编码
    ...
    OPEN o_result FOR
      SELECT iem.iem_mainpage_no,
             ...
             */
             ...
             iem.typehospital,
             iem.outhostype,
             iem.antibacterial_drugs,
             iem.durationdate,
             iem.combined_medication,
             
             iem.pathway_flag,
             iem.pathway_over,
             iem.path_out_reason,
             iem.variation_flag,
             iem.variation_reason,
             iem.rehospitalization,
             iem.intervaldate,
             ...
             iem.attending_physician_code attending_employeeID,
             attending_employee.name attending_employeeName,
      ...
      --on job.detailid = iem.jobid
        ...
        left join users resident_employee
         ...
         left join users attending_employee
          on attending_employee.id = iem.attending_physician_code
