...
namespace DrectSoft.DSSqlHelper
{
    ...
    public sealed class DS_SqlHelper
    {
        ...
        private static void CreateSqlHelper()
        {
            try
            {
                DbProviderName = ConfigurationManager.ConnectionStrings["EMRDB"].ProviderName;
                ConnectionString = ConfigurationManager.ConnectionStrings["EMRDB"].ConnectionString;
            }
            catch (Exception ce)
            {
                throw ce;
