...
[assembly: Job("住院病人和床位数据同步", "从HIS读取病人和床位数据", "移动护理", true, typeof(PatInfoSynchronous))]
namespace YidanSoft.JobManager
{
    /// <summary>
    /// 对HIS与Mcs中病人基本信息和床位数据进行同步处理
    /// </summary>
    public class PatInfoSynchronous_MCS : BaseJobAction, IDisposable
    {
        #region const
        private const string McsPatTable = "BL_BRSYK";
        private const string McsBedTable = "BL_CWDMK";
        ...
        private DataTable m_McsPatientTable;
        private DataTable m_McsBedTable;
        private IDataAccess m_McsHelper;
        ...
        public PatInfoSynchronous_MCS()
        {
            try
            {
                m_McsHelper = DataAccessFactory.GetSqlDataAccess("MCSDB");
                ...
                m_McsPatientTable = m_McsHelper.ExecuteDataTable("select * from " + McsPatTable + " where 1=2");
                m_McsHelper.ResetTableSchema(m_McsPatientTable, McsPatTable);

                m_McsBedTable = m_McsHelper.ExecuteDataTable("select * from " + McsBedTable + " where 1=2");
                m_McsHelper.ResetTableSchema(m_McsBedTable, McsBedTable);
            }
            catch
            {
                throw;
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (!this.m_disposed)
            {
                ...
                if (disposing)
                {
                    m_McsBedTable.Dispose();
                    m_McsPatientTable.Dispose();
                }
            }
            m_disposed = true;
        }

        ~PatInfoSynchronous_MCS()
        {
            Dispose(false);
        }
        #endregion
        ...
        private void InitMcsInpatientTable(bool synchChanged, string condition)
        {
            m_NewPatients.Clear();
            ...
            {
                sqlCommand = String.Format(CultureInfo.CurrentCulture, "select * from {0} where 1=1", McsPatTable);
            }

            if (!String.IsNullOrEmpty(condition))
                sqlCommand = sqlCommand + " and " + condition;

            m_McsPatientTable.Clear();

            DataTable table = m_McsHelper.ExecuteDataTable(sqlCommand, CommandType.Text);
            m_McsPatientTable.Merge(table, false, MissingSchemaAction.Ignore);
        }

        private void InitMcsBedTable()
        {
            m_NewBeds.Clear();
            InitMcsBedTable("");
        }

        private void InitMcsBedTable(string condition)
        {
            string sqlCommand = "select * from " + McsBedTable;
            ...
            DataTable table = m_McsHelper.ExecuteDataTable(sqlCommand, CommandType.Text);
            m_McsBedTable.Merge(table, false, MissingSchemaAction.Ignore);
        }
        ...
        private void UpdateRecord(DataRow sourceRow, DataRow targetRow, bool initialized, bool isPatient)
        {
            ...
            foreach (DataColumn col in sourceRow.Table.Columns)
            {
                ...
                else // 更新床位
                {
                    ...
                    // 如果记录是由空床变成占床或包床或所在的病人改变或了，则需记录下来，以便处理床位表关联的Mcs首页序号和Mcs首页库的床号
        ...
        private void DoAfterPatientStateChanged(DataRow sourceRow, DataRow targetRow)
      {
        ...
        private string MergePatientInfo(DataTable changedData, bool initialized)
        {
            ...
            foreach (DataRow hisRow in changedData.Rows)
            {
                ...
                matchRows = m_McsPatientTable.Select(String.Format(CultureInfo.CurrentCulture
                   , "hissyxh = {0}", hisRow[colHisFirstPageNo]));
                switch (matchRows.Length)
                {
                    case 0:  // 插入
                        AddNewRecord(m_McsPatientTable, hisRow, true);
                        break;
                    case 1:  // 更新
                        UpdateRecord(hisRow, matchRows[0], initialized, true);
                        break;
                    default:
                        //MessageBox.Show("Mcs中病人数据有重复");
                        break;
                }
            }
        ...
        private string MergeBedInfo(DataTable changedData, bool initialized)
        {
            ...
            foreach (DataRow hisRow in changedData.Rows)
            {
                ...
                matchRows = m_McsBedTable.Select(String.Format(CultureInfo.CurrentCulture
                   , "cwdm = '{0}' and bqdm = '{1}'", hisRow[colBedNo], hisRow["bqdm"]));
                switch (matchRows.Length)
                {
                    case 0:  // 插入
                        AddNewRecord(m_McsBedTable, hisRow, false);
                        break;
                    case 1:  // 更新
        ...
        private int SaveMcsTableData(DataTable sourceTable, string tableName)
        {
            int updatedCount = 0;
            if (sourceTable != null)
            {
                ...
                updatedCount = m_McsHelper.UpdateTable(sourceTable, tableName, true);

                // 单独处理新入院病人(因为需要获得Mcs中的syxh)
                if (m_NewPatients.Count > 0)
                {
                    ...
                    //Winning.Job.JobLogHelper.WriteSqlLog(Parent, "select syxh, hissyxh from " + McsPatTable + " where hissyxh in (" + condition.ToString() + ")");
                    DataTable newPats = m_McsHelper.ExecuteDataTable("select syxh, hissyxh from " + McsPatTable + " where hissyxh in (" + condition.ToString() + ")", CommandType.Text);
                    string key;
        ...
        private void InitPatientAndBedData(DataTable patientTable, DataTable bedTable)
        {
            ...
            if ((patientTable != null) && (patientTable.Rows.Count > 0))
            {
                ...
                {
                    // 取所有Mcs的病人数据
                    InitMcsInpatientTable(false, CreateFilterCondition(patientTable, colHisFirstPageNo));
                    // 合并，保存
                    MergePatientInfo(patientTable, true);
                    updatedCount = SaveMcsTableData(m_McsPatientTable, McsPatTable);
            ...
            if ((bedTable != null) && (bedTable.Rows.Count > 0))
            {
                try
                {
                    ...
                    updatedCount = SaveMcsTableData(m_McsBedTable, McsBedTable);
                    // 执行 syxh 的更新处理
                    //JobLogHelper.WriteSqlLog(Parent, "update " + McsBedTable + " set syxh = b.syxh from " + McsBedTable + " a, " + McsPatTable + " b where a.hissyxh = b.hissyxh");
                    //m_McsHelper.ExecuteNoneQuery("update " + McsBedTable + " set syxh = b.syxh from " + McsBedTable + " a, " + McsPatTable + " b where a.hissyxh = b.hissyxh");
                }
                catch (Exception err)
                {
                    return;
        ...
        private void SynchChangedPatientAndBedData(DataTable patientTable, DataTable bedTable)
        {
            ...
            if ((patientTable != null) && (patientTable.Rows.Count > 0))
            {
                ...
                {
                    // 取指定的Mcs的病人数据
                    InitMcsInpatientTable(true, CreateFilterCondition(patientTable, colHisFirstPageNo));
                    // 合并，保存
            ...
            if ((bedTable != null) && (bedTable.Rows.Count > 0))
            {
                try
                {
                    ...
                    // 取指定的Mcs的床位数据
                    InitMcsBedTable(condition2.ToString());
                    // 合并，保存
                    ...
                    if (m_NewBeds.Count > 0)
                    {
                        ...
                        {
                            // 更新床位表中关联的Mcs首页序号
                            objRowcount = m_McsHelper.ExecuteScalar(String.Format("update {0} set syxh = b.syxh from {0} a, {1} b where a.hissyxh = b.hissyxh and a.bqdm = '{3}' and a.cwdm = '{2}' and a.zcbz <> 1300"
                               + "\r\n"
                               + "select @@rowcount"
                               , McsBedTable, McsPatTable, para.Value[colBedNo], para.Value[colWardCode], para.Value[colDeptCode]));
                            if (objRowcount != null)
                            ...
                            //   , McsPatTable, McsBedTable, para.Key, para.Value[colWardCode], para.Value[colDeptCode]));
                            objRowcount = m_McsHelper.ExecuteScalar(String.Format("update {0} set cycw = b.cwdm from {0} a, {1} b where a.hissyxh = b.hissyxh and b.bqdm = '{3}' and b.cwdm = '{2}' and b.zcbz = 1301"
                               + "\r\n"
                               + "select @@rowcount"
                               , McsPatTable, McsBedTable, para.Value[colBedNo], para.Value[colWardCode], para.Value[colDeptCode]));
                            if (objRowcount != null)
        ...
        private DataSet GetPatientAndBedTable(bool isInit)
        {
            try
            {
                ...
                if (isInit)
                {
                    commandText = "usp_EmrDTS_GetInpatients";
