...
namespace DrectSoft.Emr.QcManagerNew
{
    public partial class UCPoint : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void InitData(IEmrHost host, SqlManger sqlManger, string noofinpat, string chiefID)
        {
        ...
        public void RefreshLookUpEditorEmrDoc(Authority auth, CheckState check, QCType type, string noofinpat, string chiefID, EmrModel emrModel, EmrModelContainer emrModelContainer)
        {
            try
            {
                ...
                InitLookUpEditorEmrDoc();//绑定病历下拉菜单列表
                InitLookUpEditorDoctor();//绑定医生
                InitTotalGrade(); //绑定底端病历评分等级
                InitGridControl();//查看评分记录详情
                InitPatientInfo();//绑定顶端病人信息
