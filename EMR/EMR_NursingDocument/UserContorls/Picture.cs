...
namespace DrectSoft.Core.EMR_NursingDocument.UserControls
{
    class Picture
    {
        ...
        /// <summary>
        /// 脉搏体温 Add By wwj 2012-07-13 如果脉搏和体温重合，改为空心黑圈 仁和医院 孙阳提出
        /// </summary>
        /// <param name="g"></param>
        /// <param name="x"></param>
        /// <param name="y"></param>
        public void DrawTiWen(Graphics g, float x, float y)
        {
            x = x - 6;
            y = y - 6;
            //g.FillEllipse(new SolidBrush(Color.White), x, y, 8f, 8f);
            //g.DrawEllipse(Pens.Black, x, y, 8f, 8f);
            g.FillEllipse(new SolidBrush(Color.Black), x, y, 10f, 10f);
            g.FillEllipse(new SolidBrush(Color.White), x + 2, y + 2, 6f, 6f);
        }
