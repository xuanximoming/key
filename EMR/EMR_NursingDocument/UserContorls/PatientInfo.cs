...
namespace DrectSoft.Core.EMR_NursingDocument.UserControls
{
    class PatientInfo
    {
        ...
        public static string InTime = DateTime.Now.ToString("yyyy-MM-dd");  //原为“入院日期”，现改为“入科日期”
        ...
        public static DataTable DataTableDayTimePoint = new DataTable();    //每天的时间点
        ...
        private void InitEventInformation()
        {
            ...
            string sql = string.Format(@"select a.dotime,b.name from PatientStatus a join THERE_CHECK_EVENT b on  a.ccode=b.id and a.noofinpat='{0}' order by dotime ", patid);
