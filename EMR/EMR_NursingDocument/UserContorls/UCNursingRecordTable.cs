    ...
    public partial class UCNursingRecordTable : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void SaveUCNursingRecordTable(string DateOfSurvey, string m_noofinpat)
        {
            ...
            where dateofsurvey='{1}' and timeslot='" + lblTime4.Text + "' and noofinpat='{2}' ", m_UcTxtSeparateBoxList[1].BP, DateOfSurvey, myNoOfInpat);
                ...
                drainage='{3}',height='{4}',weight='{5}',allergy='{6}',daysaftersurgery='{7}',dayofhospital='{8}'  
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (m_txtPulseList[i].Text.Trim().ToString() != "")
                {
                    ...
                    else
                    {
                        ...
                        {
                            MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "脉搏超出范围：20次/分至200/次/分!");
                            return false;
                        }
                    }
                }
        ...
        public void GetNotesOfNursingInfo(string DateOfSurvey, string m_oofinpat)
        {
                ...
                && Convert.ToDateTime(DateOfSurvey) == Convert.ToDateTime(DateTime.Now.Date.ToString("yyyy-MM-dd")))
            {
                ...
                string[] surgerydays;
                string searchsql = string.Format(@" select * from patientstatus where noofinpat='{0}' and
                ccode='7000'", MethodSet.CurrentInPatient.Code);
        ...
        public void SetDaysAfterSurgery(string DaysAfterSurgery, string patid)
        {
            ...
            string serachsql = string.Format(@"    select * from patientstatus where noofinpat='{0}' and trunc(to_date(patientstatus.dotime,'yyyy-mm-dd hh24:mi:ss'))>=trunc(to_date('{1}','yyyy-mm-dd hh24:mi:ss'))-14 and dotime<'{1}'  and ccode='7000' order by dotime asc", patid, DaysAfterSurgery + " 23:59:59");
