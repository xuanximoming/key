...
namespace DrectSoft.Emr.QcManager
{
    public partial class QC_LostScoreCat : UserControl, IStartPlugIn
    {
        ...
        /// 患者病历评分列表页面
        /// </summary>
        public QC_LostScoreCat(IEmrHost app)
        {
            InitializeComponent();
            m_app = app;
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DrectSoft.Common.Ctrs.DLG.MyMessageBox.Show("入院起始时间不能大于结束时间");
                    return;
                }

                if (this.dtZKStart.DateTime > this.dtZKEnd.DateTime)
                {
                    DrectSoft.Common.Ctrs.DLG.MyMessageBox.Show("质控起始时间不能大于结束时间");
                    return;
                }
                BindDataSouce();
...
and to_date('{9}', 'yyyy-MM-dd') <= r.create_time
    ...
    order by dep.id, ip.OUTBED";
                ...
                if (qctype != "-1")
                {
                    qctype = (Convert.ToInt32(qctype) - 1).ToString();
                }
