...
namespace DrectSoft.Emr.QcManager
{
    public class SqlManger
    {
        ...
        public DataTable GetPatientList(string deptNo, string patID, string name, string status, string beginInTime, string endInTime)
        {
                ...
                SqlParameter[] sqlParams = new SqlParameter[] 
            {
                ...
                if (GetConfigValueByKey("QCManagerPatientListOption") == "1")
                {
                    ...
                    {
                        return m_app.SqlHelper.ExecuteDataTable("EMRQCMANAGER.usp_getpatientlistandpatSZ_ZY", sqlParams, CommandType.StoredProcedure);
                ...
                else
                {
                    if (valueStr.ToLower().Contains("iem_mainpage_diagnosis_sx"))
                    {
                        return m_app.SqlHelper.ExecuteDataTable("EMRQCMANAGER.usp_getpatientlistandpat_ZY", sqlParams, CommandType.StoredProcedure);
                    }
                    else
                    {
       ...
       WHERE inpatient.outhosdept LIKE '%' || '{0}' || '%'
         ...
         AND (to_date(inpatient.admitdate,'yyyy-mm-dd hh24:mi:ss') between to_date('{3}','yyyy-mm-dd hh24:mi:ss') AND
            to_date('{4}','yyyy-mm-dd hh24:mi:ss'));", deptNo, patID, name, beginInTime, endInTime);
            ...
            try
            {
                ...
                if (auth == Authority.QC)
                {
                    sql = @"select rownum, t.ID ,p.name Paname, t.name Recname, t.create_time  ctime, users.name uname, t.recorddetailid recid,t.score,t.noofinpat, 
                            ...
                            (case t.QCTYPE when '0' then '环节质控' when '1' then '终末质控'when '2' then '科室质控' end) QCTYPE                            
                ...
                {
                    sql = @"select rownum, t.ID ,p.name Paname, t.name Recname, t.create_time  ctime, users.name uname, t.recorddetailid recid,t.score,t.noofinpat, 
                             (case t.CHECKSTATE when '0' then '新建' when '1' then '提交未审核' when '2' then '审核通过' when '3' then '审核未通过' when '4' then '质控员质控'when '5' then '科室质控' end) CHECKSTATE,
                            (case t.QCTYPE when '0' then '环节质控' when '1' then '终末质控'when '2' then '科室质控' end) QCTYPE                            
                            from emr_automark_record t, inpatient p,users
                            where p.noofinpat = t.noofinpat and users.id = t.create_user and t.noofinpat ='{0}'
        ...
        public string InsertAutoMarkRecord(string noofinpat, string isAuto, Authority auth, QCType qctype)
        {
            try
            {
                ...
                //非质控员质控记录
                else if (auth == Authority.DEPTMANAGER)
                {
                    sql = @"insert into emr_automark_record(id,create_time,name,create_user,recorddetailid,noofinpat,isauto,isvalid,isused,checkstate,qctype) 
                        values('{0}',sysdate, (select inp.name|| ' 评分记录' from inpatient inp where inp.noofinpat ={2}),'{1}','','{2}','{3}','1','0','5','{4}' )";
        ...
        public void InsertEmrPoint(EmrPoint emrPoint)
        {
            try
            {
                ...
                // new SqlParameter("@emrpointid", SqlDbType.Int),
                 ...
                 new SqlParameter("@EmrPointChildID", SqlDbType.VarChar),
                };
                ...
                sqlParams[13].Value = emrPoint.EmrPointChildID;
    ...
    order by CNAME,CHILDNAME,a) t");
        ...
        internal DataTable GetReductionDatadetail(string noofinapt, string chiefid)
        {
            try
            {
                //            string serchsql = string.Format(@"  
                ...
                //                                  '否'
                //                               end) validName from EMR_ConfigReduction A where A.valid='1' order by A.ID ");
                string serchsql = string.Format(@"select rownum , t.*,nvl(b.num,0) count ,nvl(b.problem_desc,t.PROBLEM_DESC)  reason ,nvl(b.num*b.reducepoint,0) point,
b.doctorid from (select rownum a,A.ID,
          A.REDUCEPOINT,
          A.PROBLEM_DESC,
          B.CHILDNAME,
          B.childcode,
    ...
    where A.valid = '1'
      ...
      and a.parents =c.CCODE 
      and a.children =b.childcode
    order by CNAME,CHILDNAME,a) t  left join (select * from  emr_point  where noofinpat='" + noofinapt + "' and emr_mark_record_id='" + chiefid + "') b on t.id=b.emrpointchildid ");
                return m_app.SqlHelper.ExecuteDataTable(serchsql);
            }
            catch (Exception)
            {
        ...
        internal DataTable GeQcTimeDatadetail(string noofinapt, string chiefid)
        {
            try
            {
                //            string serchsql = string.Format(@"  
                ...
                //                                  '否'
                //                               end) validName from EMR_ConfigReduction A where A.valid='1' order by A.ID ");
                string serchsql = string.Format(@"select '时限扣分' CNAME, '1' COUNT, t.* from emr_automark_record_detail t where t. noofinpat='{0}'and t.automarkrecordid='{1}'", noofinapt, chiefid);
                return m_app.SqlHelper.ExecuteDataTable(serchsql);
            }
            catch (Exception)
            {
