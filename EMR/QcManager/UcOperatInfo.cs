...
namespace DrectSoft.Emr.QcManager
{
    ...
    {
        IEmrHost m_app;
        public UcOperatInfo(IEmrHost app)
        {
            InitializeComponent();
            m_app = app;
        }
        ...
        private void BindData()
        {
            try
            {
                ...
                //add by ywk 去除重复数据  2013年7月16日 18:57:20
                string sql = @" select  distinct b.patid,
                           ...
                           a.admitdate,
                           a.outhosdate,
                            basic.contactaddress as address,
                           '1' qjcs,
                           '1' CGCS,
                           '' lx,
                           '未结算' fee,
                           ...
                           left join users u  on a.RESIDENT = u.id
                ...
                DataTable dt = m_app.SqlHelper.ExecuteDataTable(string.Format(sql, dept_id, begin_time, end_time, mainpage[0], mainpage[3]), CommandType.Text);//此DataTable要处理
