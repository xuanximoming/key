...
namespace DrectSoft.Core.NursingDocuments.UserControls
{
    public partial class UCNursingRecordTable : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        List<TextEdit> m_txtCountOutList = new List<TextEdit>();//出量
        ...
        //疼痛改为下拉框（数据从配置中读取）
        List<LookUpEditor> m_lookPainInfoList = new List<LookUpEditor>();//疼痛指数
        ...
        private void UCNursingRecordTable_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //根据配置判断显示引流量或者尿量2012年6月20日 10:25:46
        ...
        /// <summary>
        /// 保存护理信息数据
        /// edit by ywk 2012年5月29日 13:34:01
        /// </summary>
        ...
        public bool SaveUCNursingRecordTable(string DateOfSurvey, string m_noofinpat)
        {
            ...
            where dateofsurvey='{1}' and timeslot='" + lblTime4.Text + "' and noofinpat='{2}' ", m_UcTxtSeparateBoxList[1].BP, DateOfSurvey, myNoOfInpat);
                ...
                //更新其他栏位的值//update by wyt 20120807
        ...
        private bool IsDataValid()
        {
            try
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    //add by wyt
                    double inHosHour = GetHour(DateTime.Parse(MethodSet.AdmitDate));//入院时间点
                    double timeSlot = double.Parse(m_lblTimeList[i].Text.Trim());   //当前输入时间点
                    if ((timeSlot < inHosHour - 2) && (DateTime.Parse(MethodSet.AdmitDate)).ToShortDateString().Equals(CurrentOperTime.ToShortDateString()))
                    {
                        ...
                        if (m_txtTemperatureList[i].Text.Trim() != "" || m_txtBreatheList[i].Text.Trim() != "" || m_txtPhysicalCoolingList[i].Text.Trim() != ""
                            ...
                            || m_lookPainInfoList[i].Text.Trim() != "")
                        {
                            MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "时间点超出入院时间范围");
                            return false;
                        }
                        //if (m_UcTxtSeparateBoxList[0].IsInput && inHosHour<12) 血压先注释掉  addby ywk 2012年8月14日 11:40:19
                        //{
                        //    MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "时间点超出入院时间范围!");
                        //    return false;
                        //}
                    ...
                    if (m_txtPulseList[i].Text.Trim().ToString() != "")
                    {
                        ...
                        {
                            MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "脉搏必须为正整数");
                            return false;
                        }
                        else
                        {
                            ...
                            {
                                MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "脉搏超出范围：20次/分至200次/分。");
                                return false;
                            }
                        }
                    }
                    ...
                    if (m_txtHeartRateList[i].Text.Trim().ToString() != "")
                    {
                        ...
                        {
                            MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "心率必须为正整数");
                            return false;
                        }
                        else
                        {
                            ...
                            if (!(intVart >= 20 && intVart <= 200))
                            {
                                MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "心率超出范围：20次/分至200次/分。");
                                return false;
                            }
                        }
                    }
                    ...
                    if (m_txtBreatheList[i].Text.Trim().ToString() != "")
                    {
                        ...
                        {//不是数字
                            ...
                            {//呼吸机
                                ...
                                {
                                    MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "呼吸只能为正整数或者R(呼吸机)");
                                    return false;
                                }
                            }
                            else
                            {
                                MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "呼吸只能为正整数");
                                return false;
                            }
                        }
                        else
                        {//是数字 
                            ...
                            {
                                MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "呼吸超出范围：0次/分至90次/分。");
                                return false;
                            }
                        }
                    }
                    ...
                    //体温为空，能填写物理降温？此处先限制住
                    //edit by wyt 2012-11-05 去掉体温、物理降温、温度变化三者间的逻辑关系
                    ...
                    if (m_txtPhysicalHottingList[i].Text.Trim().ToString() != "")
                    {
                        ...
                        {
                            MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "温度变化必须为数字，小数点保留一位。");
                            return false;
                        }
                        else
                        {
                            ...
                            if (!(dVar >= 34 && dVar <= 43))
                            {
                                MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "温度变化超出范围：34℃至43℃。");
                                return false;
                            }
                        }
                    }
                    ...
                    //{
                    //    MethodSet.App.CustomMessageBox.MessageShow(strTimeSlot + "温度变化不能低于此时的体温");
                    //    return false;
                    //}
                ...
                {
                    MethodSet.App.CustomMessageBox.MessageShow("大便次数中正常排便次数、灌肠后排便次数、灌肠次数字符长度加和不能大于48");
                    return false;
                }
        ...
        private SqlParameter[] GetSqlParam(int index, string DateOfSurvey, string noofinpat)
        {
            SqlParameter[] sqlParam = new SqlParameter[]
            {
                ...
                //new SqlParameter("@indx",SqlDbType.VarChar),//序号
                ...
                //new SqlParameter("@Param1",SqlDbType.VarChar),//参数1
                //new SqlParameter("@Param2",SqlDbType.VarChar)//参数2
        ...
        public void GetNotesOfNursingInfo(string DateOfSurvey, string m_oofinpat, Inpatient currInpatient)
        {
                ...
                && Convert.ToDateTime(DateOfSurvey) == Convert.ToDateTime(DateTime.Now.Date.ToString("yyyy-MM-dd")))
            {
                ...
                (p.ccode='7000' or t.name ='分娩·手术')", currInpatient.Code);
                ...
                //给手术后天数赋值(edit by ywk 2012年5月14日 10:25:16)
                //ComputeSurgeryDay();
            }
        ...
        /// 设置手术后天数 edit by ywk
        /// </summary>
        /// <param name="DaysAfterSurgery">当前体征录入进入时的时间</param>
...
and dotime<'{1}'  and (p.ccode='7000' or t.name ='分娩·手术') order by dotime asc", patid, DaysAfterSurgery + " 23:59:59");
        ...
        /// <summary>
        /// 根据选择的日期，改变住院天数的值
        /// edit by ywk 二〇一二年五月十五日 20:48:06
        /// </summary>
