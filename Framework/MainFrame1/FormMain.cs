...
namespace DrectSoft.MainFrame
{
    public partial class FormMain : DevBaseForm, IEmrHost, ICustomMessageBox
    {
        ...
        //    colWidths.Add("Sex", 50);
        //    colWidths.Add("AgeStr", 50);
        //    colWidths.Add("PatID", 130);
        //    colWidths.Add("AdmitDate", 130);
        //    _patientWordBook = new SqlWordbook("Inpatients", PatientInfos.Tables["床位信息"].Copy(), "NoOfInpat", "PatName"
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //        XmlconfigSource = new XmlDocument();
                //        XmlconfigSource.Load(currentsoftpath + "Emr.exe.config");
                //        XmlNodeList SourcenodeList = XmlconfigSource.SelectNodes("/configuration/connectionStrings");
                //        SourcenodeList[0].InnerXml = newstr;
                //        XmlconfigSource.Save(currentsoftpath + "Emr.exe.config");
                //        GOLogin();
                //    }
                //    else
                //    {
                ...
                if (Appinfo.Count > 0)
                {
                    ...
                    //  DrectSoft.FrameWork.IPlugIn plg = this.Manager.Runner.LoadPlugIn("DrectSoft.Core.OwnBedInfo.dll", "DrectSoft.Core.OwnBedInfo.BedMappingStartUp", true);
                    ////  this.CurrentPatientInfo.NoOfHisFirstPage = Appinfo[1].ToString();
                ...
                //  if (plg == null) this.CustomMessageBox.MessageShow("加载" + ((PlugInConfiguration)mould.Tag).MenuCaption + "失败！");
        ...
        private void GetHospitalInfo()
        {
            try
            {
                ...
                if (dtHos.Rows.Count > 0)
                {
                    ...
                    hospitalinfo.Name = /*dr["Name"].ToString(); by Ukey 2016-08-25 */DrectSoft.MainFrame.PublicClass.getHosName();/*医院名称*/
        ...
        //        sqlParams[1].Value = this.User.Id.Trim(); // 当前登录人编码

        //        dt = this.SqlHelper.ExecuteDataTable("usp_GetMyConsultion", sqlParams, CommandType.StoredProcedure);
        ...
        //                string.Format(" stateid in ('6730') and applyuser='{0}' and applydept='{1}'", this.User.Id, this.User.CurrentDeptId)
        //            );
        ...
        //            #region 受邀人能看到 待会诊，会诊保存，会诊完成

        //            #region 会诊单中指定的受邀人
        //            Lookrow = dt.Select(string.Format(" stateid in ('6730') and EmployeeCode='{0}' and departmentcode='{1}' ", this.User.Id, this.User.CurrentDeptId));
        //            for (int i = 0; i < Lookrow.Length; i++)
        //            {
        //                string applySn = Lookrow[i]["consultapplysn"].ToString();
        //                if (!applySnList.Contains(applySn))
        ...
        //            Lookrow = dt.Select(string.Format(" stateid in ('6730','6740','6741') and EmployeeCode is null and departmentcode='{0}' and EmployeeLevelID = '{1}'", this.User.CurrentDeptId, emp.Grade));
        //            for (int i = 0; i < Lookrow.Length; i++)
        //            {
        //                string applySn = Lookrow[i]["consultapplysn"].ToString();
        //                if (!applySnList.Contains(applySn))
